...
class VideoTracker:
    ...
    def _reset_tracking_state(self) -> None:
        ...
        self._trajectories: dict[int, list[tuple[int, int]]] = defaultdict(list)
    ...
    def _update_track_state(
        ...
        if stat.last_center is not None:
            stat.path_length += float(math.hypot(cx - stat.last_center[0], cy - stat.last_center[1]))
    ...
    def _update_with_deepsort(
        ...
        for track in tracks:
            ...
            self._update_track_state(track_id, cls_name, cls_id, frame_index, track_box)
    ...
    def update(
        ...
        if self.deepsort_enabled and self._deep_sort is not None:
            try:
                return self._update_with_deepsort(detections, frame_bgr, frame_index)
            except _DeepSortAssociationFailure:
                return self._update_with_naive(detections, frame_index)
            except Exception as exc:
                logger.exception("DeepSort tracking failed")
                self._disable_deepsort(exc)
        return self._update_with_naive(detections, frame_index)

    def get_trajectories(self, active_within_frames: int | None = None) -> dict[int, list[tuple[int, int]]]:
    ...
    def get_serialized_trajectories(self, active_within_frames: int | None = None) -> dict[str, list[dict[str, int]]]:
        serialized: dict[str, list[dict[str, int]]] = {}
        for track_id, points in self.get_trajectories(active_within_frames=active_within_frames).items():
            serialized[str(track_id)] = [{"x": int(x), "y": int(y)} for x, y in points]
        return serialized
...
class CameraTrackerManager:
    ...
    def update_session(
        ...
        with self._lock:
            ...
            tracked_boxes = session.tracker.update(detections, frame_bgr, session.frame_index)
